#!/usr/bin/env python3
"""D51 Wait DAG V1: build rank0 active-window causal graph from frozen V2b."""
...
ANALYZER_VERSION = "wait_dag_v1_slice_a"
...
class WaitDagBuilder:
    ...
    def build(self) -> dict[str, Any]:
        ...
        compute_streams = freeze_compute_streams(
            self.db_path, self.active_start, self.active_end, string_ids
        )
        comm_cids = {op["connection_id"] for op in comm_ops}
        fifo_ambiguous_streams = fifo_ambiguous_stream_ids(
            stream_tasks, self.active_start, self.active_end
        )
        ...
        # --- FIFO edges ---
        fifo_ambiguous_count = 0
        for sid, stasks in stream_tasks.items():
            active_on_stream = [
                t for t in stasks if self.active_start <= t.start_ns <= self.active_end
            ]
            issues = detect_fifo_ambiguity(active_on_stream)
            if issues:
                fifo_ambiguous_count += 1
                anchor = task_node_id(active_on_stream[0].rowid) if active_on_stream else f"stream:{sid}"
                self._add_unknown(
                    anchor,
                    "profiler_same_stream_fifo",
                    "adjacent_task",
                    "fifo_order_ambiguous",
                    ";".join(issues[:3]),
                    "unambiguous profiler order",
                    False,
                    True,
                    False,
                )
                continue
            for i in range(len(active_on_stream) - 1):
                a, b = active_on_stream[i], active_on_stream[i + 1]
                self.causal_edges.append(
                    CausalEdge(
                        edge_id=self._next_edge_id("fifo"),
        ...
        coverage = {
            ...
            "profiler_same_stream_fifo": {
                "expected": "per_stream_adjacent_pairs",
                "proven": 0,
                "observed_structural": fifo_count,
                "unknown": fifo_ambiguous_count,
                "not_observed": 0,
                "denominator_source": "profiler_stream_fifo",
            },
        ...
        acceptance = {
            ...
            "passed": passed and len(contradiction) == 0,
        }
...
def export_graph(
    ...
    graph = {
        ...
        "claims": {
            "allowed": build_allowed_claims(build_meta["coverage"]),
