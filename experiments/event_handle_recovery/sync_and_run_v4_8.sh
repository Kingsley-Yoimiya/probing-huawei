#!/usr/bin/env bash
# Sync V4.8 sources to npu-dev-1, run fresh RUN, pull myportal evidence (even on STOP).
set -euo pipefail
...
REMOTE_DIR="/root/event_handle_recovery_v48"
...
echo "Sync V4.8 -> ${REMOTE_HOST}:${CONTAINER}:${REMOTE_DIR}"
...
tar -C "${SRC}" -cf - \
  ...
  run_d51_wait_dag_v4_8.sh smoke_post_wait_v4_7.py \
...
ssh "${SSH_QUIET[@]}" "${REMOTE_HOST}" \
  "sudo docker exec ${CONTAINER} bash -lc 'cd ${REMOTE_DIR} && chmod +x run_d51_wait_dag_v4_8.sh kernels/build_kernel.sh build.sh && RUN_ID=${RUN_ID} BUILD_ID=${BUILD_ID} ./run_d51_wait_dag_v4_8.sh'" \
...
for f in STOP.json gate_summary.json hash_manifest.json package_receipt.json frozen_dose_v4_5_ref.json \
  paired_effects_v4_8.csv post_wait_injection_audit.csv intervention_identity.csv kernel_realization.csv \
  comm_entry_projection.csv node_wallclock.csv control_effects_v4_8.csv capture_classification.csv \
  run_ledger.csv claims.md preflight.json capture_contract.json smoke_post_wait.json v4_8_summary.json; do
  pull_file "${f}"
done
