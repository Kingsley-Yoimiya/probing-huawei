#!/usr/bin/env bash
# Sync V4.4 sources to npu-dev-1 container (no container restart).
set -euo pipefail
...
echo "Sync V4.4 -> ${REMOTE_HOST}:${CONTAINER}:${REMOTE_DIR}"
...
tar -C "${SRC}" -cf - \
  ...
  run_d51_wait_dag_v4_4.sh run_d51_wait_dag_v4_3.sh run_v4_1_artifact_probe.sh \
  ...
  tests/test_wait_dag_v4_2.py tests/test_wait_dag_v4_3.py tests/test_wait_dag_v4_4.py \
