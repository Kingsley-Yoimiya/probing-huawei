#!/usr/bin/env bash
# D51 Wait DAG V4.10: C3 ordinal studentized envelope + global location gate.
set -euo pipefail
...
write_terminal_payload() {
  local gate_file="$1"
  GATE_FILE="${gate_file}" WORKDIR="${WORKDIR}" RUN_ID="${RUN_ID}" BUILD_ID="${BUILD_ID}" \
  UTC_STAMP="${UTC_STAMP}" KERNEL_SHA="${KERNEL_SHA:-unknown}" \
  CAPTURE_SHA="${CAPTURE_SHA:-unknown}" C3_CONTRACT_SHA="${C3_CONTRACT_SHA:-unknown}" \
  python3 - <<'PY'
import datetime, json, os
from pathlib import Path

gate_path = Path(os.environ["GATE_FILE"])
gate_in = json.loads(gate_path.read_text()) if gate_path.exists() else {}
workdir = Path(os.environ["WORKDIR"])
finalizer_epoch = datetime.datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
payload = {
    "stop": gate_in.get("stop"),
    "utc": os.environ["UTC_STAMP"],
    "run_id": os.environ["RUN_ID"],
    "build_id": os.environ["BUILD_ID"],
    "finalizer_epoch": finalizer_epoch,
    "contract_sha": os.environ.get("C3_CONTRACT_SHA", "unknown"),
    "capture_contract_sha256": os.environ.get("CAPTURE_SHA", "unknown"),
    "kernel_sha256": os.environ.get("KERNEL_SHA", "unknown"),
    "inject_site": "AFTER_SUCCESSFUL_TARGET_WAIT",
    "fail_block": gate_in.get("fail_block"),
    "control_reason": gate_in.get("control_reason"),
    "fail_run": gate_in.get("fail_run"),
    "b1_protocol_pass": bool(gate_in.get("b1_protocol_pass", False)),
    "dlarge_3_of_3_pass": bool(gate_in.get("dlarge_3_of_3_pass", False)),
    "b2_dlarge_pass": bool(gate_in.get("b2_dlarge_pass", False)),
    "b3_dlarge_pass": bool(gate_in.get("b3_dlarge_pass", False)),
    "dsmall_structure_only": bool(gate_in.get("dsmall_structure_only", False)),
    "unit_pass": bool(gate_in.get("unit_pass", True)),
    "smoke_pass": bool(gate_in.get("smoke_pass", True)),
}
for k in ("pairs", "classification", "block", "dlarge_pass"):
    if k in gate_in:
        payload[k] = gate_in[k]
text = json.dumps(payload, indent=2) + "\n"
for name in ("gate_summary.json", "STOP.json"):
    tmp = workdir / f"{name}.tmp"
    auth = workdir / name
    tmp.write_text(text)
...
atomic_write_stop() {
  local reason="$1"
  local fail_block="${2:-}"
  local gate_file="${LOGDIR}/analysis/stop_gate.json"
  python3 - <<PY
import json
from pathlib import Path
gate = {
...
Path("${gate_file}").write_text(json.dumps(gate) + "\n")
PY
  write_terminal_payload "${gate_file}"
  log "STOP ${reason}"
}
...
promote_package_artifacts() {
  ...
  for f in paired_effects_v4_10.csv c3_ordinal_rank_pairs_v4_10.csv c3_pseudotarget_scores_v4_10.csv \
    c3_studentized_envelope_v4_10.csv c3_global_location_v4_10.csv \
    c5_path_diagnostics.csv control_effects_v4_10.csv capture_classification.csv run_ledger.csv \
    v4_10_summary.json path_external_proofs.json path_external_proofs.csv; do
...
write_stop_null() {
  local gate_file="${WORKDIR}/gate_summary.json"
  python3 - <<PY
import json
from pathlib import Path
p = Path("${gate_file}")
gate = json.loads(p.read_text()) if p.exists() else {}
gate["stop"] = None
...
if ! python3 "${ROOT}/tests/test_wait_dag_v4_10.py" 2>&1 | tee "${LOGDIR}/build_unit/pytest_v4_10.log"; then
  atomic_write_stop "STOP_UNIT_PYTEST_V410_FAILED"
  exit 3
fi
...
from wait_dag_v4_10_intervention import C3_CONTRACT, C5_DIAG, c3_contract_sha
root = Path("${ROOT}")
def sha(p):
    h=hashlib.sha256()
    with open(p,'rb') as f:
...
print(json.dumps({
  ...
  "wait_dag_v4_10_sha256": sha(root/"wait_dag_v4_10_intervention.py"),
  ...
  "c3_preflight": {
    "r_star": 0,
    "sentinel_ranks": list(range(1,16)),
    "K": 8,
    "lambda": 3,
    "epsilon_ns": 1,
    "quartile_rule": "nearest_rank_Q1_a4_Q3_a12",
  },
...
C3_CONTRACT_SHA=$(python3 -c "import json; print(json.load(open('${LOGDIR}/preflight/preflight.json'))['c3_contract_sha256'])")
...
GATE_RC=0
if ! python3 "${ROOT}/wait_dag_v4_10_intervention.py" \
  ...
  set +e
  GATE_JSON_FILE="${LOGDIR}/analysis/gate_eval_raw.json"
  ...
  python3 - <<'PY' > "${GATE_JSON_FILE}"
import csv, json, os
from pathlib import Path
pair_out = Path(os.environ["PAIR_OUT"])
rows = list(csv.DictReader((pair_out / "paired_effects_v4_10.csv").open())) if (pair_out / "paired_effects_v4_10.csv").exists() else []
...
PY
  GATE_RC=0
  if python3 -c "import json,sys; g=json.load(open('${GATE_JSON_FILE}')); sys.exit(8 if g.get('stop') else 0)"; then
    :
  else
    GATE_RC=8
  fi
  write_terminal_payload "${GATE_JSON_FILE}" | tee "${LOGDIR}/analysis/gate_eval.json"
  set -e
fi

package_myportal() {
  log "Step package: publish tree then manifest (immutable)"
  local pair_src="${PAIR_OUT:-${WORKDIR}/analysis/paired_all}"
  promote_package_artifacts "${pair_src}"
  mkdir -p "${MYPORTAL}/logs/${UTC_STAMP}"

  local publish_roots=(
    paired_effects_v4_10.csv c3_ordinal_rank_pairs_v4_10.csv c3_pseudotarget_scores_v4_10.csv
    c3_studentized_envelope_v4_10.csv c3_global_location_v4_10.csv
    c5_path_diagnostics.csv control_effects_v4_10.csv capture_classification.csv run_ledger.csv
    v4_10_summary.json path_external_proofs.json path_external_proofs.csv
    gate_summary.json STOP.json frozen_dose_v4_5_ref.json
    preflight.json capture_contract.json smoke_post_wait.json
  )
  for f in "${publish_roots[@]}"; do
    if [[ -f "${WORKDIR}/${f}" && -s "${WORKDIR}/${f}" ]]; then
      cp "${WORKDIR}/${f}" "${MYPORTAL}/"
    elif [[ -f "${LOGDIR}/preflight/${f}" && -s "${LOGDIR}/preflight/${f}" ]]; then
      cp "${LOGDIR}/preflight/${f}" "${MYPORTAL}/"
    elif [[ -f "${LOGDIR}/real_smoke/${f}" && -s "${LOGDIR}/real_smoke/${f}" ]]; then
      cp "${LOGDIR}/real_smoke/${f}" "${MYPORTAL}/"
    elif [[ -f "${MYPORTAL}/${f}" && -s "${MYPORTAL}/${f}" ]]; then
      :
    else
  ...
  sync "${LOGDIR}/runner.log" 2>/dev/null || true
  cp -a "${LOGDIR}/." "${MYPORTAL}/logs/${UTC_STAMP}/" 2>/dev/null || true
  log "BUILD_V4_10_COMPLETE RUN_ID=${RUN_ID} KERNEL_SHA=${KERNEL_SHA}"
  exec > >(tee -a "${LOGDIR}/package/finalizer.log") 2>&1

  local pkg_log="${LOGDIR}/package/sync.log"
  MYPORTAL="${MYPORTAL}" WORKDIR="${WORKDIR}" RUN_ID="${RUN_ID}" BUILD_ID="${BUILD_ID}" \
  python3 - <<'PY' > "${pkg_log}"
import hashlib, json, datetime, os, sys
from pathlib import Path

myportal = Path(os.environ["MYPORTAL"])
workdir = Path(os.environ["WORKDIR"])
for p in sorted(myportal.rglob("*")):
    ...
    if p.stat().st_size == 0:
        continue
    rel = p.relative_to(myportal)
    wdst = workdir / rel
    wdst.parent.mkdir(parents=True, exist_ok=True)
    data = p.read_bytes()
    if not wdst.exists() or wdst.read_bytes() != data:
        wdst.write_bytes(data)
manifest = {"run_id": os.environ["RUN_ID"], "build_id": os.environ["BUILD_ID"], "files": []}
for p in sorted(myportal.rglob("*")):
    if not p.is_file():
        continue
    if p.name == "hash_manifest.json":
        continue
    if p.stat().st_size == 0:
        print(f"SKIP_ZERO_BYTE:{p.relative_to(myportal)}", file=sys.stderr)
        continue
...
for entry in manifest["files"]:
    ...
    for base in (myportal, workdir):
        ...
        if not p.is_file() or p.stat().st_size != entry["bytes"]:
            mismatch.append(f"{base.name}:{rel}")
            break
        h = hashlib.sha256()
        with open(p, "rb") as f:
            for chunk in iter(lambda: f.read(1 << 20), b""):
        ...
        if h.hexdigest() != entry["sha256"]:
            mismatch.append(f"{base.name}:{rel}:sha")
            break
if mismatch:
    print(json.dumps({"sync_mismatch": mismatch[:20], "count": len(mismatch)}), file=sys.stderr)
    sys.exit(12)
...
(workdir / "hash_manifest.json").write_text(manifest_path.read_text())
(workdir / "package_receipt.json").write_text((myportal / "package_receipt.json").read_text())
print(json.dumps({"hash_manifest": str(manifest_path), "file_count": manifest["file_count"]}, indent=2))
PY
  local pkg_rc=$?
  if [[ ! -s "${pkg_log}" ]]; then
    echo "SKIPPED:package_sync_empty UTC=$(date -u +%Y%m%dT%H%M%SZ)" > "${pkg_log}"
  fi
  if [[ "${pkg_rc}" -eq 12 ]]; then
    atomic_write_stop "STOP_PACKAGE_OR_SYNC_MISMATCH"
    return 12
  fi
}
...
PY
  ...
  set +e
  local gate_file="${LOGDIR}/analysis/gate_eval_${block}.json"
  PAIR_OUT="${out_dir}" BLOCK="${block}" \
  python3 - <<'PY' > "${gate_file}"
import csv, json, os
from pathlib import Path
pair_out = Path(os.environ["PAIR_OUT"])
block = os.environ["BLOCK"]
rows = list(csv.DictReader((pair_out / "paired_effects_v4_10.csv").open()))
cls_rows = list(csv.DictReader((pair_out / "capture_classification.csv").open()))
gate = {"block": block, "stop": None, "dlarge_pass": False, "dlarge_3_of_3_pass": False}
for cr in cls_rows:
    if cr.get("pre_wait_p_status") != "STRUCTURAL_NA":
        gate["stop"] = cr.get("pre_wait_p_status") or "STOP_PRE_WAIT_P_CLASSIFICATION_FAILED"
        gate["fail_run"] = cr.get("run_id")
    ...
    for r in rows:
        ...
        if r.get("structure_gate_pass") != "True":
            gate["stop"] = "STOP_STRUCTURE_GATE_FAILED"
            gate["fail_block"] = f"{block}_dlarge"
            break
        if r.get("dose_gate_pass") != "True":
            gate["stop"] = "STOP_DOSE_GATE_FAILED"
            gate["fail_block"] = f"{block}_dlarge"
...
PY
  local rc=0
  if python3 -c "import json,sys; g=json.load(open('${gate_file}')); sys.exit(8 if g.get('stop') else 0)"; then
    :
  else
    rc=8
  fi
  write_terminal_payload "${gate_file}" | tee "${gate_file}.terminal"
  set -e
  PAIR_OUT="${out_dir}"
  return "${rc}"
}
...
set +e
GATE_JSON_FILE="${LOGDIR}/analysis/gate_eval_final_raw.json"
PAIR_OUT="${PAIR_OUT}" \
python3 - <<'PY' > "${GATE_JSON_FILE}"
import csv, json, os
from pathlib import Path
...
write_terminal_payload "${GATE_JSON_FILE}" | tee "${LOGDIR}/analysis/gate_eval_final.json"
set -e
