#!/usr/bin/env bash
# D51 Wait DAG V3.1 — structure-key paired causal intervention (fresh evidence only).
set -euo pipefail
...
RUN_ID="${RUN_ID:-${UTC_STAMP}_d51_wait_dag_v3_1}"
WORKDIR="/tmp/${RUN_ID}"
...
# --- preflight ---
{
  ...
  pgrep -a vllm || true
  sha256sum "${ROOT}/event_interpose.cpp" "${ROOT}/wait_dag_v3_intervention.py" 2>/dev/null || true
...
if not r.get("normalized_structure_key"):
    raise SystemExit("missing structure key")
...
if not id_rows or int(id_rows[0].get("match_count", 0)) != 1:
    raise SystemExit("D2 identity match_count != 1")
print("D2_PILOT_OK callback_ms", cb_ms)
PY
...
claims = [
    "# claims (V3.1 — pending dual Auditor)",
    "",
    f"- RUN_ID: {run_id}",
    "- Old V3 package **not** used in statistics.",
    "",
]
for p in paired:
    st = p.get("status", "OK")
...
for row in csv.DictReader(open(ad / "intervention_identity.csv")):
    casebook.append(
        ...
        f"- cs record={row['preload_record_cs']} wait={row['preload_wait_cs']} (run-local diagnostic)\\n"
...
for tag in control_d0_b1 control_d0_b2 control_d0_b3 small_d2ms effect_d25_b1 effect_d25_b2 effect_d25_b3; do
  audit="$(find "${WORKDIR}/${tag}/event_trace" -name '*.delay_audit.json' 2>/dev/null | head -1)"
  if [[ -n "${audit}" ]]; then
    cp "${audit}" "${MYPORTAL}/delay_audit_${tag}.json"
  fi
done
