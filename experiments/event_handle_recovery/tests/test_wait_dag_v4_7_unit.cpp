...
void FinalizeScenario(const std::string& dir) {
...
aclrtEvent g_ev = nullptr;

int EnsureEvent() {
    if (g_ev != nullptr) {
        return 0;
    }
    return aclrtCreateEvent(&g_ev) != 0 ? 1 : 0;
}
...
int RunScenario(const char* name) {
    const std::string dir =
        std::string("/tmp/v4_7_unit_") + name + "_" + std::to_string(getpid());
    std::filesystem::remove_all(dir);
    std::filesystem::create_directories(dir);
    setenv("ACL_EVENT_TRACE_DIR", dir.c_str(), 1);
    if (std::strcmp(name, "wrong_rank_no_launch") == 0) {
        setenv("RANK", "1", 1);
    ...
    if (std::strcmp(name, "double_wait_single_launch") == 0) {
        setenv("ACL_EVENT_WORK_ITERS", "325", 1);
        acl_event_delay_arm();
        SuccessRecords(1);
        SuccessWaits(2);
        FinalizeScenario(dir);
        WorkAudit a{};
        if (!LoadWorkAudit(dir, &a) || a.launch_count != 1) {
            return 9;
        }
        return 0;
    }
  if (std::strcmp(name, "zero_iters_armed_no_launch") == 0) {
        setenv("ACL_EVENT_WORK_ITERS", "0", 1);
        acl_event_delay_arm();
        SuccessRecords(1);
        SuccessWaits(1);
...
int main() {
    const std::vector<const char*> cases = {
        ...
        "fail_wait_no_launch",
        "wrong_ordinal",
        "wrong_rank_no_launch",
        "double_wait_single_launch",
        "zero_iters_armed_no_launch",
    };
    int failed = 0;
