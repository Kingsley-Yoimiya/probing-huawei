...
unset PROBING PROBING_TORCH_PROFILING PROBING_GPU INLINE_INJECT 2>/dev/null || true; export PROBING=0; export INLINE_INJECT=2c; export INLINE_VICTIM_LOCAL_RANK=7; export INLINE_INJECT_START=100; export INLINE_INJECT_STOP=300; export INLINE_2C_N=768; export INLINE_2C_EVERY=4; export INLINE_2C_FALLBACK_S=0.1;
OUT='/afs-a3-weight-share/yinjinrun.p-huawei/results/ascend-ais/20260726_021606-yjr-as-c-p1-sw-c-quiet/P1-SW-C/by_pod/grj-megatron-32card-0716-master-0/round_1/C1_inject_none'
rm -f "$OUT/node_0.done" "$OUT/node_0.fail"
rm -rf "$OUT/ranks"
mkdir -p "$OUT/ranks"
/root/miniconda3/envs/llm_test/bin/torchrun --nnodes=1 --nproc_per_node=16 --node_rank=0 \
  --master_addr=10.119.0.183 --master_port=30100 \
  ...
  --run-id=20260726_021606-yjr-as-c-p1-sw-c-quiet --group=1 --config='C1_inject_none' --round=1 \
