#!/usr/bin/env bash
# PR-2 实验 A5：step_duration_sec 列名修复 · P3-SW-A GT=rank7
set -euo pipefail
ROOT="/Users/yinjinrun/Codespace/myportal/project/probing-huawei"
TS="${RUN_ID_TS:-$(date +%Y%m%d_%H%M%S)}"
export PARENT_RUN_ID="${PARENT_RUN_ID:-${TS}-pillar-c-v3-pr2-localize-a5}"
...
# 相对 A4：更长训程以覆盖 inject 窗；DUMP_WAIT 仍短于训程
export ITERS=1800
export WARMUP=50
...
export PILLAR_C_LOCALIZE_PARALLEL=16
