#!/usr/bin/env bash
# PR-2 实验 A3 (R3)：attach 就绪 + localize 重试 · P3-SW-A GT=rank7
set -euo pipefail
ROOT="/Users/yinjinrun/Codespace/myportal/project/probing-huawei"
TS="${RUN_ID_TS:-$(date +%Y%m%d_%H%M%S)}"
export PARENT_RUN_ID="${PARENT_RUN_ID:-${TS}-pillar-c-v3-pr2-localize-a3}"
...
# R3：attach 就绪 + 重试
export PILLAR_C_LOCALIZE_TIMEOUT_S=25
export PILLAR_C_LOCALIZE_RETRIES=3
export PILLAR_C_LOCALIZE_RETRY_PAUSE_S=5
export PILLAR_C_ATTACH_READY_WAIT_S=45
export PILLAR_C_ATTACH_RETRY_INTERVAL_S=2
export PILLAR_C_ATTACH_RETRIES=6
