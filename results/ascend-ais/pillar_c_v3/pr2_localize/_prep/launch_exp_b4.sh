#!/usr/bin/env bash
# PR-2 实验 B4：B3 修复验证 — pod 内原生 SET_DOWNGRADE_OK + jexec_poll Mac 无 GNU timeout
set -euo pipefail
ROOT="/Users/yinjinrun/Codespace/myportal/project/probing-huawei"
TS="${RUN_ID_TS:-$(date +%Y%m%d_%H%M%S)}"
export PARENT_RUN_ID="${PARENT_RUN_ID:-${TS}-pillar-c-v3-pr2-e3-b4}"
...
# B4：30s 时基降回（R3 用 30s；验证 pod 内 probing rate=0）
...
cat >"${PARENT_LOCAL}/PR2_EXP_B4_LAUNCH.md" <<EOF
# PR-2 实验 B4 · 发射记录
...
| scope | localize + B4 时基 \`${PILLAR_C_SET_WINDOW_S}\`s（steps=\`${PILLAR_C_SET_WINDOW_STEPS}\`） |
...
## 发射
\`_prep/launch_exp_b4.sh\` @ $(date -Iseconds)
EOF

cp "${PARENT_LOCAL}/PR2_EXP_B4_LAUNCH.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_EXP_B4_LAUNCH.md"

echo "[pr2-b4] self-check test_pillar_c_set_window…"
bash "${ROOT}/scripts/fail-slow/test_pillar_c_set_window.sh"

echo "[pr2-b4] jexec_poll smoke (Mac 无 GNU timeout)…"
_jexec_poll_smoke() {
  local t="$1"; shift
...
if command -v timeout >/dev/null 2>&1 || command -v gtimeout >/dev/null 2>&1; then
  echo "[pr2-b4] WARN: GNU timeout present — smoke still runs python path via forced branch"
fi
# 强制走 python 分支
python3 - 5 true <<'PY'
import subprocess, sys
...
sys.exit(rc)
PY
echo "[pr2-b4] jexec_poll python-timeout smoke: exit=$?"
...
if [[ "$elapsed_s" -lt "$window_s" ]]; then
  echo "[pr2-b4] window_s smoke: elapsed=${elapsed_s}s < ${window_s}s (no trigger yet) OK"
else
  echo "[pr2-b4] window_s smoke: would trigger SET_DOWNGRADE reason=time"
fi

echo "[pr2-b4] jsync scripts → bundle (HOLD_EXEC_SKIP_HEAVY_JSYNC=1)…"
jsync_one() {
  local src="$1" dst="$2"
  ...
  rc=$?
  set -e
  echo "[pr2-b4] jsync $bname -> $dst rc=$rc"
  return 0
}
...
if echo "$idle_out" | grep -q OWNER_BUSY; then
  echo "[pr2-b4] BLOCKED: pod busy"
  echo "BLOCKED pod_busy" >"${PARENT_LOCAL}/BLOCKED.txt"
  exit 90
fi

echo "[pr2-b4] fire dynamic arm ${ARM_RUN_ID}…"
cd "${ROOT}"
...
# 原生 SET_DOWNGRADE_OK 检查（拒绝 mac_triggered_backfill）
native_dg=0
backfill=0
...
if grep -q 'HANG_DETECTED' "${PARENT_LOCAL}/dynamic/"**/set_upgrade.log 2>/dev/null \
  ...
  cat >"${PARENT_LOCAL}/PR2_EXP_B4_STATUS.md" <<MD
# PR-2 实验 B4 · **BLOCKED**
...
MD
  cp "${PARENT_LOCAL}/PR2_EXP_B4_STATUS.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_EXP_B4_STATUS.md"
  echo "[pr2-b4] BLOCKED hang before downgrade"
  exit 2
fi
...
path=${FULL_REF}
case=${CASE_ID}
note=B4 reuse v2 full_fidelity upper bound
EOF
...
python3 "${ROOT}/scripts/fail-slow/pr2_e3_score_ratio.py" \
  ...
  --out-json "${PARENT_LOCAL}/PR2_E3_RATIO_B4.json" \
  --out-md "${PARENT_LOCAL}/PR2_E3_RATIO_B4.md"

cp "${PARENT_LOCAL}/PR2_E3_RATIO_B4.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_E3_RATIO_B4.md"
cp "${PARENT_LOCAL}/PR2_E3_RATIO_B4.json" "${LOCAL_RESULT_ROOT_BASE}/PR2_E3_RATIO_B4.json"

HEADLINE=$(python3 -c "import json; print(json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B4.json'))['headline_pct'])" 2>/dev/null || echo "?")
DENSE=$(python3 -c "import json; d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B4.json')); print(d['dynamic'].get('torch_trace_dense_ranks','?'))" 2>/dev/null || echo "?")
CULPRIT=$(python3 -c "import json; d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B4.json')); print(d['dynamic'].get('localize',{}).get('culprit_rank','?'))" 2>/dev/null || echo "?")
SET_DG=$(grep -h 'SET_DOWNGRADE_OK' "${PARENT_LOCAL}/dynamic/"**/set_upgrade.log 2>/dev/null | wc -l | tr -d ' ')
...
# 原生判定：有 SET_DOWNGRADE_OK 且无 backfill 标记
NATIVE_OK=no
...
VERDICT=PARTIAL
HEADLINE_OK=0
if python3 -c "import json; h=float(json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B4.json'))['headline_pct']); exit(0 if h<=80 else 1)" 2>/dev/null; then
  HEADLINE_OK=1
fi
...
if [[ -f "$CODE_STATUS" ]]; then
  if ! grep -q "实验 B4" "$CODE_STATUS"; then
    cat >>"$CODE_STATUS" <<MD
...
|------|--------|----------|-----------|------|
| **B4** | \`${PARENT_RUN_ID}\` | culprit=**${CULPRIT}** | **${HEADLINE}%** | SET_DG=${SET_DG} native=${NATIVE_OK} dense=${DENSE} |
MD
  fi
fi
...
echo "[pr2-b4] ${VERDICT} headline=${HEADLINE}% dense=${DENSE} culprit=${CULPRIT} SET_DG=${SET_DG} native=${NATIVE_OK}"
echo "$PARENT_RUN_ID"
