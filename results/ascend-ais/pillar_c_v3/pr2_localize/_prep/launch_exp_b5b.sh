#!/usr/bin/env bash
# PR-2 实验 B5b：B5 + jsync ext/torch C0 热更 → culprit dense=1
set -euo pipefail
ROOT="/Users/yinjinrun/Codespace/myportal/project/probing-huawei"
TS="${RUN_ID_TS:-$(date +%Y%m%d_%H%M%S)}"
export PARENT_RUN_ID="${PARENT_RUN_ID:-${TS}-pillar-c-v3-pr2-e3-b5b}"
...
cat >"${PARENT_LOCAL}/PR2_EXP_B5b_LAUNCH.md" <<EOF
# PR-2 实验 B5b · 发射记录
...
## 发射
\`_prep/launch_exp_b5b.sh\` @ $(date -Iseconds)
EOF
cp "${PARENT_LOCAL}/PR2_EXP_B5b_LAUNCH.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_EXP_B5b_LAUNCH.md"

echo "[pr2-b5b] self-check test_pillar_c_set_window…"
bash "${ROOT}/scripts/fail-slow/test_pillar_c_set_window.sh"

echo "[pr2-b5b] ext/torch.py C0 hot-sync self-check…"
...
if [[ "${TORCH_LINES}" -lt 150 ]]; then
  echo "[pr2-b5b] BLOCKED: ext/torch.py too short (${TORCH_LINES} lines)"
  exit 91
fi
echo "ext/torch.py C0 OK (${TORCH_LINES} lines)"

echo "[pr2-b5b] inline rate=0 plan smoke…"
python3 - <<'PY'
import os
...
jsync_one() {
  ...
  local rc=$?
  set -e
  echo "[pr2-b5b] jsync $bname -> $dst rc=$rc"
  return 0
}

echo "[pr2-b5b] jsync scripts + torch_probe + ext/torch → bundle…"
...
echo "[pr2-b5b] verify bundle ext/torch.py on pod…"
ssh -o BatchMode=yes -o ConnectTimeout=30 "${JUMP_HOST}" \
...
if echo "$idle_out" | grep -q OWNER_BUSY; then
  echo "[pr2-b5b] BLOCKED: pod busy"
  echo "BLOCKED pod_busy" >"${PARENT_LOCAL}/BLOCKED.txt"
  exit 90
fi

echo "[pr2-b5b] fire dynamic arm ${ARM_RUN_ID}…"
cd "${ROOT}"
...
if grep -q 'HANG_DETECTED' "${PARENT_LOCAL}/dynamic/"**/set_upgrade.log 2>/dev/null \
  ...
  cat >"${PARENT_LOCAL}/PR2_EXP_B5b_STATUS.md" <<MD
# PR-2 实验 B5b · **BLOCKED**

- parent: \`${PARENT_RUN_ID}\`
- 现象: SET 后 stall 且未 SET_DOWNGRADE
MD
  cp "${PARENT_LOCAL}/PR2_EXP_B5b_STATUS.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_EXP_B5b_STATUS.md"
  exit 2
fi
...
path=${FULL_REF}
case=${CASE_ID}
note=B5b reuse v2 full_fidelity upper bound
EOF
...
python3 "${ROOT}/scripts/fail-slow/pr2_e3_score_ratio.py" \
  ...
  --out-json "${PARENT_LOCAL}/PR2_E3_RATIO_B5b.json" \
  --out-md "${PARENT_LOCAL}/PR2_E3_RATIO_B5b.md"

cp "${PARENT_LOCAL}/PR2_E3_RATIO_B5b.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_E3_RATIO_B5b.md"
cp "${PARENT_LOCAL}/PR2_E3_RATIO_B5b.json" "${LOCAL_RESULT_ROOT_BASE}/PR2_E3_RATIO_B5b.json"

HEADLINE=$(python3 -c "import json; print(json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5b.json'))['headline_pct'])" 2>/dev/null || echo "?")
DENSE=$(python3 -c "import json; d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5b.json')); print(d['dynamic'].get('torch_trace_dense_ranks','?'))" 2>/dev/null || echo "?")
CULPRIT=$(python3 -c "import json; d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5b.json')); print(d['dynamic'].get('localize',{}).get('culprit_rank','?'))" 2>/dev/null || echo "?")
CULPRIT_ROWS=$(python3 -c "
import json
d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5b.json'))
...
VERDICT=PARTIAL
HEADLINE_OK=0
if python3 -c "import json; h=float(json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5b.json'))['headline_pct']); exit(0 if h<100 else 1)" 2>/dev/null; then
  HEADLINE_OK=1
fi
...
| B5b 改动 | ext/torch C0 热更 + torch_probe rate=0 |
...
if [[ -f "$CODE_STATUS" ]]; then
  if ! grep -q "实验 B5b" "$CODE_STATUS"; then
    cat >>"$CODE_STATUS" <<MD

## 实验 B5b 状态（ext/torch C0 热更）

| 轮次 | run_id | localize | E3 头条比 | dense | culprit_rows | 备注 |
|------|--------|----------|-----------|-------|--------------|------|
| **B5b** | \`${PARENT_RUN_ID}\` | culprit=**${CULPRIT}** | **${HEADLINE}%** | **${DENSE}** | **${CULPRIT_ROWS}** | SET_DG=${SET_DG} native=${NATIVE_OK} |
MD
  fi
fi
...
- **run**：\`${PARENT_RUN_ID}\`
- **verdict**：${VERDICT}
- **dense**：${DENSE} · **culprit**：${CULPRIT} · **culprit_rows**：${CULPRIT_ROWS} · **headline**：${HEADLINE}%
...
| B5 | 20260728_130247 | 114.99% | 0 | 7 | 零行 OK；ext/torch 未 jsync |
| **B5b** | ${PARENT_RUN_ID} | **${HEADLINE}%** | **${DENSE}** | **${CULPRIT}** | ext/torch C0 热更 |
MD
fi

echo "[pr2-b5b] ${VERDICT} headline=${HEADLINE}% dense=${DENSE} culprit_rows=${CULPRIT_ROWS} culprit=${CULPRIT} SET_DG=${SET_DG}"
echo "$PARENT_RUN_ID"
