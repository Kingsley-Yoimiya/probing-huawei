#!/usr/bin/env bash
# PR-2 实验 B5d：B5c PASS 后全训 · WINDOW_S=15 · hang_max=480s
set -euo pipefail
ROOT="/Users/yinjinrun/Codespace/myportal/project/probing-huawei"
TS="${RUN_ID_TS:-$(date +%Y%m%d_%H%M%S)}"
export PARENT_RUN_ID="${PARENT_RUN_ID:-${TS}-pillar-c-v3-pr2-e3-b5d}"
...
cat >"${PARENT_LOCAL}/PR2_EXP_B5d_LAUNCH.md" <<EOF
# PR-2 实验 B5d · 发射记录
...
## 发射
\`_prep/launch_exp_b5d.sh\` @ $(date -Iseconds)
EOF
cp "${PARENT_LOCAL}/PR2_EXP_B5d_LAUNCH.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_EXP_B5d_LAUNCH.md"

echo "[pr2-b5d] self-check test_pillar_c_set_window…"
bash "${ROOT}/scripts/fail-slow/test_pillar_c_set_window.sh"

echo "[pr2-b5d] ext/torch.py C0 hot-sync self-check…"
...
if [[ "${TORCH_LINES}" -lt 150 ]]; then
  echo "[pr2-b5d] BLOCKED: ext/torch.py too short (${TORCH_LINES} lines)"
  exit 91
fi
...
jsync_one() {
  ...
  local rc=$?
  set -e
  echo "[pr2-b5d] jsync $bname -> $dst rc=$rc"
  return 0
}

echo "[pr2-b5d] jsync scripts + torch_probe + ext/torch → bundle…"
...
echo "[pr2-b5d] verify bundle ext/torch.py on pod…"
ssh -o BatchMode=yes -o ConnectTimeout=30 "${JUMP_HOST}" \
...
if echo "$idle_out" | grep -q OWNER_BUSY; then
  echo "[pr2-b5d] BLOCKED: pod busy"
  echo "BLOCKED pod_busy" >"${PARENT_LOCAL}/BLOCKED.txt"
  exit 90
fi

echo "[pr2-b5d] fire dynamic arm ${ARM_RUN_ID}…"
cd "${ROOT}"
...
if grep -q 'HANG_DETECTED' "${PARENT_LOCAL}/dynamic/"**/set_upgrade.log 2>/dev/null \
  ...
  cat >"${PARENT_LOCAL}/PR2_EXP_B5d_STATUS.md" <<MD
# PR-2 实验 B5d · **BLOCKED**

- parent: \`${PARENT_RUN_ID}\`
- 现象: SET 后 stall 且未 SET_DOWNGRADE
MD
  cp "${PARENT_LOCAL}/PR2_EXP_B5d_STATUS.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_EXP_B5d_STATUS.md"
  exit 2
fi
...
path=${FULL_REF}
case=${CASE_ID}
note=B5d reuse v2 full_fidelity upper bound
EOF
...
python3 "${ROOT}/scripts/fail-slow/pr2_e3_score_ratio.py" \
  ...
  --out-json "${PARENT_LOCAL}/PR2_E3_RATIO_B5d.json" \
  --out-md "${PARENT_LOCAL}/PR2_E3_RATIO_B5d.md"

cp "${PARENT_LOCAL}/PR2_E3_RATIO_B5d.md" "${LOCAL_RESULT_ROOT_BASE}/PR2_E3_RATIO_B5d.md"
cp "${PARENT_LOCAL}/PR2_E3_RATIO_B5d.json" "${LOCAL_RESULT_ROOT_BASE}/PR2_E3_RATIO_B5d.json"

HEADLINE=$(python3 -c "import json; print(json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5d.json'))['headline_pct'])" 2>/dev/null || echo "?")
DENSE=$(python3 -c "import json; d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5d.json')); print(d['dynamic'].get('torch_trace_dense_ranks','?'))" 2>/dev/null || echo "?")
CULPRIT=$(python3 -c "import json; d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5d.json')); print(d['dynamic'].get('localize',{}).get('culprit_rank','?'))" 2>/dev/null || echo "?")
CULPRIT_ROWS=$(python3 -c "
import json
d=json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5d.json'))
...
VERDICT=PARTIAL
HEADLINE_OK=0
if python3 -c "import json; h=float(json.load(open('${PARENT_LOCAL}/PR2_E3_RATIO_B5d.json'))['headline_pct']); exit(0 if h<100 else 1)" 2>/dev/null; then
  HEADLINE_OK=1
fi
...
| B5d 改动 | B5c PASS 后全训 · 15s 窗 · 8min hang |
...
if [[ -f "$CODE_STATUS" ]]; then
  if ! grep -q "实验 B5d" "$CODE_STATUS"; then
    cat >>"$CODE_STATUS" <<MD
...
|------|--------|----------|-----------|-------|--------------|------|
| **B5d** | \`${PARENT_RUN_ID}\` | culprit=**${CULPRIT}** | **${HEADLINE}%** | **${DENSE}** | **${CULPRIT_ROWS}** | WINDOW=15s hang=480s SET_DG=${SET_DG} |
MD
  fi
fi
...
| B5c smoke | 20260728_135724 | — | — | 7 | rows=4368 PASS |
| **B5d** | ${PARENT_RUN_ID} | **${HEADLINE}%** | **${DENSE}** | **${CULPRIT}** | WINDOW=15s 全训 |
MD

echo "[pr2-b5d] ${VERDICT} headline=${HEADLINE}% dense=${DENSE} culprit_rows=${CULPRIT_ROWS} culprit=${CULPRIT} SET_DG=${SET_DG}"
echo "$PARENT_RUN_ID"
