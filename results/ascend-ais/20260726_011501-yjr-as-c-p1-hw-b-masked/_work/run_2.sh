...
export PROBING=2; export PROBING_GPU=on; export PROBING_GPU_BACKEND=npu; export PROBING_NPU_SOURCE=auto; export PROBING_GPU_SAMPLE_MS=1000; export PROBING_CPU=on; export PROBING_CPU_SAMPLE_MS=1000; export PYTHONPATH=/afs-a3-241ceshi-shared/yinjinrun.p-huawei/probe-bundle/pydeps:${PYTHONPATH:-}; export PATH=/afs-a3-241ceshi-shared/yinjinrun.p-huawei/probe-bundle/pydeps/bin:/root/miniconda3/envs/llm_test/bin:${PATH}; unset PROBING_TORCH_PROFILING; unset PROBING_COLD_MAX_TOTAL_MB 2>/dev/null || true; unset PROBING_COLD 2>/dev/null || true; export INLINE_INJECT=hbm; export INLINE_VICTIM_LOCAL_RANK=7; export INLINE_INJECT_START=100; export INLINE_INJECT_STOP=300; export INLINE_HBM_MB=256; export INLINE_HBM_COPIES=4; export INLINE_HBM_COPIES_MAX=24; export INLINE_HBM_RAMP=1; export INLINE_HBM_RAMP=1;
OUT='/afs-a3-weight-share/yinjinrun.p-huawei/results/ascend-ais/20260726_011501-yjr-as-c-p1-hw-b-masked/P1-HW-B/by_pod/grj-megatron-32card-0716-master-0/round_1/C2_probing'
...
/root/miniconda3/envs/llm_test/bin/torchrun --nnodes=1 --nproc_per_node=16 --node_rank=0 \
  ...
  --run-id=20260726_011501-yjr-as-c-p1-hw-b-masked --group=2 --config='C2_probing' --round=1 \
