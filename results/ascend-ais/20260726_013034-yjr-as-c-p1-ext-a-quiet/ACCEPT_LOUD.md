# Acceptance (loud): P1-EXT-A

- window: measure step [100, 300] rank0 `step_ms` median
- dose: `loud`
- threshold C1/C0 ≥ **1.15** (cli:--min-ratio=1.15)
- recipes: `/tmp/yjr_fs_orch_20260726_013034/huawei_root/scripts/fail-slow/dose_recipes.yaml`
- injection.log: `started`
- verdict: **PASS**

| config | median step_ms | vs C0 |
|---|---:|---:|
| C0_baseline | 76.10 | 1.00 |
| C1_inject_none | 88.00 | 1.16 |
| C2_probing | 89.10 | 1.17 |

C1/C0 = 1.156
C2/C0 = 1.171
