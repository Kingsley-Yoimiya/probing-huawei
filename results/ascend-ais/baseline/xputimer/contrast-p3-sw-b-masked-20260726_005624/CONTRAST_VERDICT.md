# XPUTimer contrast · P3-SW-B masked

- case_id: `P3-SW-B`
- case_ref: `20260726_000113-yjr-as-c-p3-sw-b-masked`
- dose: `masked` — INLINE 8b mb=6 stall_s=0.1 victim=7; window [100,300]; gold≈1.909
...
|-----|--:|-------------:|------------------------------:|
| C0  | 70336 | 124.0 | 9972 |
| C1  | 70336 | 127.0 | 10553 |

**C1/C0 coll ratio = 1.024** → FAIL (thr 1.05)

> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 9972 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 142.921 (n=3200)
- C1 median step_ms: 179.378 (n=3200)
- C1/C0 step_ms = 1.255 → PASS (thr 1.05)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=1.024；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=1.255)
