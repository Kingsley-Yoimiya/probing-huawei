# XPUTimer contrast · P1-SW-A quiet

- case_id: `P1-SW-A`
- case_ref: `20260726_042922-yjr-as-c-p1-sw-a-quiet`
- dose: `quiet` — INLINE 2a chunks=3 stall_mb=128 stall_s=0.05 victim=7; window [100,300]; gold≈1.638
...
> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 1634 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 75.543 (n=3200)
- C1 median step_ms: 124.428 (n=3200)
- C1/C0 step_ms = 1.647 → PASS (thr 1.15)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=1.009；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=1.647)
