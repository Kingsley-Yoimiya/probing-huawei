# XPUTimer contrast · P3-EXT-A quiet

- case_id: `P3-EXT-A`
- case_ref: `20260726_075912-yjr-as-c-p3-ext-a-quiet`
- dose: `quiet` — stress_cpu cpu_n=128 cpu_load=70 victim=7; window [100,300]; gold≈1.256
...
|-----|--:|-------------:|------------------------------:|
| C0  | 70336 | 125.0 | 9400 |
| C1  | 70336 | 132.0 | 13516 |

**C1/C0 coll ratio = 1.056** → FAIL (thr 1.15)

> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 9400 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 126.669 (n=3200)
- C1 median step_ms: 158.031 (n=3200)
- C1/C0 step_ms = 1.248 → PASS (thr 1.15)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=1.056；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=1.248)
