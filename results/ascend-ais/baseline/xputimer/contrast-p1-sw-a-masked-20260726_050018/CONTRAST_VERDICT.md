# XPUTimer contrast · P1-SW-A masked

- case_id: `P1-SW-A`
- case_ref: `20260726_044454-yjr-as-c-p1-sw-a-masked`
- dose: `masked` — INLINE 2a chunks=1 stall_mb=64 stall_s=0.02 victim=7; window [100,300]; gold≈1.259
...
> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 6095 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 76.064 (n=3200)
- C1 median step_ms: 94.577 (n=3200)
- C1/C0 step_ms = 1.243 → PASS (thr 1.05)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=0.983；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=1.243)
