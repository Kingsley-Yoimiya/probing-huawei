# XPUTimer contrast · P3-SW-A masked

- case_id: `P3-SW-A`
- case_ref: `20260725_224156-yjr-as-c-p3-sw-a-masked`
- dose: `masked` — INLINE 8a gc_every=1 stall_s=0.05 victim=7; window [100,300]
...
|-----|--:|-------------:|------------------------------:|
| C0  | 70336 | 123.0 | 9845 |
| C1  | 70336 | 127.0 | 11168 |

**C1/C0 coll ratio = 1.033** → FAIL (thr 1.05)

> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 9845 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 151.845 (n=3200)
- C1 median step_ms: 237.531 (n=3200)
- C1/C0 step_ms = 1.564 → PASS (thr 1.05)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=1.033；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=1.564)
