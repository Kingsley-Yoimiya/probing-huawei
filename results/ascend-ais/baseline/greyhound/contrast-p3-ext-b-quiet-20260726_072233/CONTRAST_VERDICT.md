...
- run_id: `contrast-p3-ext-b-quiet-20260726_072233`
- case_ref: `20260726_065841-yjr-as-c-p3-ext-b-quiet`
- dose_label: `quiet`；dose: stress_io fio_nj=4,iodepth=16,bs=4k,size=1G,ckpt_every=50,io_read_kb=256; engine=fio; window [100,300]; gold≈1.709；窗对齐 Case [100,300]
- fairness: `collect_seq` 真实 per-rank 序列 + C0 假阳性对照
- detect_ok: **no**；detect_mode: **no_bite**（自主= coll 比≥1.15 或 Rbeast 变点[C1有/C0无]）
- oracle_trigger: **no**（未把注入窗写入判定）；注入窗 [100,300] 仅标注；step_ms 窗比=1.020（剂量核对，非 Greyhound 规则）
...
**C1/C0 coll ratio = 1.015** → FAIL (thr 1.15)
...
|-----|---------:|--------:|---------:|-----------:|---------------:|
| C0  | 639915 | 4401 | 10 | 8 | 0 |
| C1  | 644386 | 4401 | 10 | 8 | 0 |
...
## Runtime notes
- io_engine=fio-3.29 (bundle from AFS via jump); held≈31–38s; BW≈19.5MiB/s
- dose_check step_ms=1.020 dose_WEAK（金标≈1.709@grj formal）；未改对手阈值
- prior stress-ng attempt  also dose_WEAK（保留作对照）
