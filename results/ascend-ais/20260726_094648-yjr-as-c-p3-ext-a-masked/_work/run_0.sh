...
OUT='/afs-a3-weight-share/yinjinrun.p-huawei/results/ascend-ais/20260726_094648-yjr-as-c-p3-ext-a-masked/P3-EXT-A/by_pod/grj-megatron-32card-0716-master-0/round_1/C0_baseline'
rm -f "$OUT/node_0.done" "$OUT/node_0.fail"
rm -rf "$OUT/ranks"
mkdir -p "$OUT/ranks"
/root/miniconda3/envs/llm_test/bin/torchrun --nnodes=1 --nproc_per_node=16 --node_rank=0 \
  --master_addr=10.119.0.183 --master_port=30000 \
  ...
  --run-id=20260726_094648-yjr-as-c-p3-ext-a-masked --group=0 --config='C0_baseline' --round=1 \
