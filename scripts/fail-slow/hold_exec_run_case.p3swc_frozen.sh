...
source "/Users/yinjinrun/Codespace/myportal/project/probing-huawei/scripts/fail-slow/env.sh"
...
case "$CASE_ID" in
  ...
  P3-SW-C)
    ...
    # nproc@90 咬合但杀训练（134528 step≈199 死）；默认 cpu_n=160 留核给 torchrun
    INJECT_KIND="${INJECT_KIND:-8c}"
    MODE="${MODE_OVERRIDE:-host_bound}"
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.3}"
    HOST_BOUND_MATMUL="${HOST_BOUND_MATMUL:-768}"
    SIDECAR_8C_CPU_N="${SIDECAR_8C_CPU_N:-160}"
    SIDECAR_8C_CPU_LOAD="${SIDECAR_8C_CPU_LOAD:-90}"
    SIDECAR_8C_MB="${SIDECAR_8C_MB:-1}"
    SIDECAR_8C_LEAK_EVERY="${SIDECAR_8C_LEAK_EVERY:-1.0}"
    SIDECAR_8C_MAX_CHUNKS="${SIDECAR_8C_MAX_CHUNKS:-64}"
    INJECT_ARGS="${INJECT_ARGS:-cpu_n=${SIDECAR_8C_CPU_N},cpu_load=${SIDECAR_8C_CPU_LOAD},mb=${SIDECAR_8C_MB},leak_every=${SIDECAR_8C_LEAK_EVERY},max_chunks=${SIDECAR_8C_MAX_CHUNKS}}"
    ;;
  P1-SW-A)
