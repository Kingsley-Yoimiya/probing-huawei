#!/usr/bin/env python3
"""P2-SW-C Loud 验收：拓扑映射漂移。

主证优先 C1/C0 **comm_ms**（EXTRA_AR 落 comm 窗）；step 作辅。
任一侧 ≥ min_ratio → PASS（对齐 P2-SW-B：step 弱不自动 FAIL）。
...
def main() -> int:
    ...
    ap.add_argument("--case", default="P2-SW-C")
    ...
    ap.add_argument("--ineffective-below", type=float, default=1.08)
    args = ap.parse_args()
    ...
    primary = None
    primary_name = ""
    if comm_r is not None and step_r is not None:
        if comm_r >= step_r:
            primary, primary_name = comm_r, "comm_ms"
        else:
            primary, primary_name = step_r, "step_ms"
    elif comm_r is not None:
        primary, primary_name = comm_r, "comm_ms"
    ...
    if primary is None:
        ...
        ok = False
    elif primary >= min_ratio:
        verdict = "PASS"
        ok = True
    elif primary < args.ineffective_below:
    ...
    if inj_logs:
        ...
        if "topo_5c" in text.lower() or "TOPO_EXTRA_AR" in text:
            inj = "topo_5c"
        else:
            inj = "log_present"

    lines = [
        f"# Loud acceptance: {args.case} (topo_5c; primary=max(step,comm))",
        "",
        f"- window: measure step [{args.lo}, {args.hi}] rank0",
        f"- threshold max(C1/C0 step, C1/C0 comm) ≥ **{min_ratio}**",
        f"- injection.log: `{inj}`",
        f"- verdict: **{verdict}** (winner=`{primary_name or 'NA'}`)",
        "",
    ...
    lines += [
        ...
        f"- step_ratio={step_r}",
        f"- comm_ratio={comm_r}",
        f"- **primary_ratio={primary}** ({primary_name}) ← 主证",
    ]
    md = "\n".join(lines) + "\n"
    print(md)
    if args.write_md:
