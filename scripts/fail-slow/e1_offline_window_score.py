...
def render_md(results: list[dict[str, Any]]) -> str:
    lines = [
        ...
        "3. 按 case 判「够归因」：",
        "   - **P3-SW-A/B**：主证 `cpu.utilization` RSS **窗内抬升**（≥50MiB；离线定 W* 不用纯绝对值）；辅看按步 `allocated`。",
        "   - **P1-HW-B**：仅计注入窗内 `allocated/cached/max_allocated` 斜率（≥256MiB）；禁 cold-start 跳变。",
        "   - **P1-SW-C**：`torch_trace` post-forward `duration` 尖刺（≥3×中位且 ≥0.4s）。",
        "4. **W*** = 首次 enough=true 的最小 W；`UNRESOLVED`/`NO_W_STAR` ≠ 用 cold MiB 冒充。",
        "",
        "## W* 总表",
        "",
