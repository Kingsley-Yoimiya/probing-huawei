...
> **现役主线已切 E1–E4**：见 [`../PILLAR_C_QUEUE.md`](../PILLAR_C_QUEUE.md) 与 [`PILLAR_C_RUNNER.md`](PILLAR_C_RUNNER.md)。  
> GATE G1–G6 **已全绿**（2026-07-25）；**不必重跑 Pilot**，除非换 so/镜像。  
> **注意**：G3=Y 只证 `torch.profiling` **配置可读回**；SET→**live tracer** 仍属 C0（`MECH_FIX.md`），未绿勿宣称升详臂有效。
...
- 跳板 kubectl=`/root/.cache/volcano/kubectl/kubectl`；优先 **自升 16 卡**；备选 grj-w0（IDLE+让路）。`yysong` 已不可用。
