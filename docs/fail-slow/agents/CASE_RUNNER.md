...
- **运行模式**：优先 **自升一台 16 卡**（`yjr-*`）；备选 `grj-megatron-32card-0716-master-0`（确认 IDLE + 让路）。**`yysong` 已不可用**，勿默认 hold-exec
- **结果/日志标签**：`yjr-as-c-*`；自有 vcjob 跑完 **suspend 不 delete**；**禁止**碰 `a3-megatron-*`；禁止写宋盘 / 删他人 vcjob
...
- 因「allocatable 空闲=0」停跑；**删/改** `yysong` / `grj-megatron-*` vcjob（除非用户明确要求）  
- 写宋一扬 AFS / `/afs-a3-241ceshi-shared/yysong`  
- 把已不可用的 `yysong` 当默认池、或未确认 IDLE 就蹭 GRJ  
...
优先自升一台 16 卡（yjr-*）；GRJ 仅确认 IDLE 才 hold-exec + 让路。yysong 已不可用。勿碰 a3；勿删他人 vcjob。落盘 yinjinrun.p-huawei → $LOCAL_RESULT_ROOT_BASE。
